DROP FUNCTION IF EXISTS public.bootstrap_admin(uuid);

DROP FUNCTION IF EXISTS public.importar_faturas_lote(jsonb, date);

CREATE OR REPLACE FUNCTION public.importar_faturas_lote(p_registros jsonb, p_vencimento date, p_actor uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
DECLARE
  v_clientes_afetados int := 0;
  v_faturas_criadas int := 0;
  v_faturas_atualizadas int := 0;
  v_ignorados int := 0;
BEGIN
  IF p_actor IS NULL OR NOT private.has_role(p_actor, 'admin'::app_role) THEN
    RAISE EXCEPTION 'Apenas administradores podem importar faturas.';
  END IF;

  IF p_vencimento IS NULL THEN
    RAISE EXCEPTION 'Informe a data de vencimento.';
  END IF;

  CREATE TEMP TABLE tmp_import ON COMMIT DROP AS
  SELECT DISTINCT ON (telefone)
    telefone, nome, email, documento, observacoes,
    valor_original, valor_desconto, status
  FROM (
    SELECT
      CASE
        WHEN length(d.tel) IN (12, 13) AND left(d.tel, 2) = '55' THEN substring(d.tel from 3)
        ELSE d.tel
      END AS telefone,
      NULLIF(btrim(COALESCE(d.r->>'nome','')), '')        AS nome,
      NULLIF(btrim(COALESCE(d.r->>'email','')), '')       AS email,
      NULLIF(btrim(COALESCE(d.r->>'documento','')), '')   AS documento,
      NULLIF(btrim(COALESCE(d.r->>'observacoes','')), '') AS observacoes,
      COALESCE((d.r->>'valor_original')::numeric, 0)      AS valor_original,
      COALESCE((d.r->>'valor_desconto')::numeric, 0)      AS valor_desconto,
      CASE
        WHEN COALESCE(NULLIF(btrim(COALESCE(d.r->>'status','')), ''), 'em_aberto') IN
          ('em_aberto','paga','vencida','cancelada','expirada','falhou','em_processamento')
        THEN COALESCE(NULLIF(btrim(COALESCE(d.r->>'status','')), ''), 'em_aberto')::fatura_status
        ELSE 'em_aberto'::fatura_status
      END AS status
    FROM (
      SELECT r, regexp_replace(COALESCE(r->>'telefone',''), '\D', '', 'g') AS tel
      FROM jsonb_array_elements(p_registros) AS r
    ) d
  ) s
  WHERE length(s.telefone) BETWEEN 10 AND 11;

  SELECT jsonb_array_length(p_registros) - (SELECT count(*) FROM tmp_import)
    INTO v_ignorados;

  WITH upserted AS (
    INSERT INTO public.clientes (nome, telefone, email, documento, observacoes)
    SELECT COALESCE(nome, telefone), telefone, email, documento, observacoes
    FROM tmp_import
    ON CONFLICT (telefone) DO UPDATE SET
      nome        = COALESCE(EXCLUDED.nome, public.clientes.nome),
      email       = COALESCE(EXCLUDED.email, public.clientes.email),
      documento   = COALESCE(EXCLUDED.documento, public.clientes.documento),
      observacoes = COALESCE(EXCLUDED.observacoes, public.clientes.observacoes),
      updated_at  = now()
    RETURNING id, telefone
  )
  SELECT count(*) INTO v_clientes_afetados FROM upserted;

  CREATE TEMP TABLE tmp_alvo ON COMMIT DROP AS
  SELECT
    c.id AS cliente_id,
    GREATEST(t.valor_original, t.valor_desconto) AS valor_original,
    t.valor_desconto,
    t.status,
    (
      SELECT f.id FROM public.faturas f
      WHERE f.cliente_id = c.id
        AND f.status IN ('em_aberto','vencida','expirada','falhou','em_processamento')
      ORDER BY f.vencimento DESC
      LIMIT 1
    ) AS fatura_id
  FROM tmp_import t
  JOIN public.clientes c ON c.telefone = t.telefone;

  WITH atualizadas AS (
    UPDATE public.faturas f
    SET valor_original = a.valor_original,
        valor_desconto = a.valor_desconto,
        vencimento     = p_vencimento,
        status         = a.status,
        updated_at     = now()
    FROM tmp_alvo a
    WHERE f.id = a.fatura_id
    RETURNING f.id
  )
  SELECT count(*) INTO v_faturas_atualizadas FROM atualizadas;

  WITH criadas AS (
    INSERT INTO public.faturas (cliente_id, descricao, valor_original, valor_desconto, vencimento, status)
    SELECT a.cliente_id, 'Fatura importada', a.valor_original, a.valor_desconto, p_vencimento, a.status
    FROM tmp_alvo a
    WHERE a.fatura_id IS NULL
    RETURNING id
  )
  SELECT count(*) INTO v_faturas_criadas FROM criadas;

  DROP TABLE IF EXISTS tmp_import;
  DROP TABLE IF EXISTS tmp_alvo;

  RETURN jsonb_build_object(
    'clientes', v_clientes_afetados,
    'faturas_criadas', v_faturas_criadas,
    'faturas_atualizadas', v_faturas_atualizadas,
    'ignorados', v_ignorados
  );
END;
$function$;

REVOKE ALL ON FUNCTION public.importar_faturas_lote(jsonb, date, uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.importar_faturas_lote(jsonb, date, uuid) TO service_role;